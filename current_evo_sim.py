...
def reproduce_and_mutate(herbs, best_herb, overide=False):
    if len(herbs) <= 40 or overide:
        print("reproduced :)")
        new_herbs = [Herbivore(random.randint(0, screen_width), random.randint(0, screen_height), neural_network=best_herb.neural_network) for _ in range(30)]
...
def reproduce_and_mutate_pred(preds, best_pred, overide=False):
    if len(preds) <= 20 or overide:
        print("reproduced :)")
...
def visualize_neural_network(neural_network):
    ...
    input_labels = ['food_x', 'food_y', 'food2_x', 'food2_y', 'pred_x', 'pred_y', 'color', 'color2']
...
def main():
    herbs = [Herbivore(random.randint(0, screen_width), random.randint(0, screen_height)) for _ in range(80)]
    carns = [Carnivore(random.randint(0, screen_width), random.randint(0, screen_height)) for _ in range(50)]
    plants = []
    best = None
    best_carn = None
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_1:
                    reproduce_and_mutate(herbs, best, overide=True)
                if event.key == pygame.K_2:
                    reproduce_and_mutate_pred(carns, best_carn, overide=True)
